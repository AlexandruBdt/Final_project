...
def income_category_summary(request):
    ...
    revenue = UserIncome.objects.filter(owner=request.user, date__gte=six_months_ago, date__lte=todays_date)
    finalrep = {}

    source_list = list(set(map(lambda income: income.source, revenue)))

    def get_income_source_amount(source):
        amount = 0
        filtered_by_source = revenue.filter(source=source)
